...
namespace HoloLab.PositioningTools.CoordinateSystem
{
    public class WorldCoordinateBinderWithLocationService : MonoBehaviour
    {
        ...
        {
            var forward = subReferencePoint.ApplicationPose.position - referencePoint.ApplicationPose.position;
            forward.y = 0;
            var cameraRotation = Quaternion.LookRotation(forward, Vector3.up);

            var applicationPose = new Pose(referencePoint.ApplicationPose.position, cameraRotation);
